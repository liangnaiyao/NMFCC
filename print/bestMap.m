...
Label1 = unique(L1); % 测试对象
nClass1 = length(Label1); 
Label2 = unique(L2); % 标签 ，预测的标签和分割的标签由可能是少的
nClass2 = length(Label2);
% %----    注释掉的方法，只有数据集类别不平衡时才用。
% if numel(Label1)==numel(Label2)
%     nClass2 = length(Label2);
% elseif numel(Label1) <  numel(Label2) 
%     nClass2 = length(Label1);
% elseif numel(Label1) >  numel(Label2) 
%     nClass2 = length(Label2);
%     nClass1= length(Label2);
% end    
% %----
